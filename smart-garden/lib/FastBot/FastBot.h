/**
 * @brief This library is a copied from the original library of the same name by Ivan Grokhotkov.
 * Source: https://github.com/GyverLibs/FastBot
 */

#ifndef _FastBot_h
#define _FastBot_h

//#define FB_USE_CALLBACK
...
public:
...
#ifdef FB_USE_CALLBACK

    /**
     * @brief Construct a new FastBot object
     * @param token bot token
     * @param limit limit of last messages
     * @param ovf overflow flag
...
#ifdef FB_USE_CALLBACK
    /**
     * @brief skip all updates message in the queue on next poll
     */
    void skipUpdates() {
        ID = -1;
    }
    ...
    /**
     * @brief Get new message from Telegram server immediately
     * @return
    uint8_t tickManual() {
    ...
    /**
     * @brief Get new message from Telegram server every _prd milliseconds
     * @return
     */
    uint8_t tick() {
        if (!*_callback) return 7;
    ...
    uint8_t
    editFile(uint8_t *buf, uint32_t length, FB_FileType type, const String &name, int32_t msgid, const String &id) {
...
#define FB_SECURE_CLIENT WiFiClientSecure
    ...
    // ============================ MULTIPART SEND ============================
    bool
    _multipartSend(FB_SECURE_CLIENT &client, uint32_t length, FB_FileType type, const String &name, const String &id) {
        ...
        switch (type) {
            case FB_PHOTO:
                client.print(F("/sendPhoto"));
                break;
            case FB_AUDIO:
                client.print(F("/sendAudio"));
                break;
            case FB_DOC:
                client.print(F("/sendDocument"));
                break;
            case FB_VIDEO:
                client.print(F("/sendVideo"));
                break;
            case FB_GIF:
                client.print(F("/sendAnimation"));
                break;
            case FB_VOICE:
                client.print(F("/sendVoice"));
                break;
        }
    ...
    // ============================ MULTIPART EDIT ============================
    bool _multipartEdit(FB_SECURE_CLIENT &client, uint32_t length, FB_FileType type, const String &name, uint32_t msgid,
                        const String &id) {
        if (!client.connect("api.telegram.org", 443)) return 0;
        String startReq;
...
#define FB_DECLARE_CLIENT() \
    ...
    // ============================ EDIT BUF ============================
    uint8_t
    _editFile(uint8_t *buf, uint32_t length, FB_FileType type, const String &name, int32_t msgid, const String &id) {
        FB_DECLARE_CLIENT();
    ...
    uint8_t parseMessages(const String &str) {
        ...
        while (true) {
            ...
            IDpos = str.indexOf(F("{\"update_id\":"),
                                IDpos + 1);   // позиция id СЛЕДУЮЩЕГО обновления (мы всегда на шаг впереди)
            if (IDpos < 0)
                IDpos = str.length();                    // если конец пакета - для удобства считаем что позиция ID в конце
            if (skip) continue;
